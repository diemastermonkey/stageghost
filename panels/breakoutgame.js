...
function fnTumble (argCol, argRow) {
  ...
  oFrom = aBricks[argCol][argRow].position; // BUG: IS UNDEFINED HERE
  oFrom = {x:0, y:0, z:0}; // DEBUG KLUDGE
  ...
  oBrickTween.onUpdate (function() {
    // 
    // ANSWER - WHY BROKEN! argCol, argRow are
    // UNDEFINED during tween cycle! Use (ugh) globals
    //
    // aBricks[argCol][argRow].position.y = oFrom.y;
    // aBricks[argCol][argRow].rotation.z -= 0.3;
    // 
    aBricks[iTweenCol][iTweenRow].position.y = oFrom.y;
    aBricks[iTweenCol][iTweenRow].rotation.z -= 0.3;
  ...
  oBrickTween.start();     // Actually start tween
}                         // End function
